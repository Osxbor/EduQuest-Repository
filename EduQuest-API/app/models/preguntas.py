...
class Preguntas(db.Model):
    ...
    examen_id = db.Column(db.Integer, db.ForeignKey('examenes.id', ondelete='CASCADE'))
...
#__repr__ es un método de Python para representar instancias de la clase Preguntas, es una forma "formal" para definir las instancias
#Se utilizó este método específicamente para tener una representación del objeto fácil de leer para el depurador
    def __repr__(self):#self se refiere a la instancia (objeto) actual de Preguntas, como llamarlo.
        #La f'<String>' Es una CADENA FORMATEADA se encarga de "construir" la representación del objeto. 
        #{self.nombre} accede al tributo nombre de la clase Preguntas, con el fin de tener (cosntruir) una representación personalizada
        #del objeto, tomando info específica sobre el propio objeto.
        return f'<Preguntas {self.pregunta}>'